...
translation_matrix = m4.create_from_translation(pyrr.Vector3(-center))

# Create the scaling matrix
scale_factor = 2.0 / dia
scaling_matrix = m4.create_from_scale(pyrr.Vector3([scale_factor] * 3))

# Create the rotation matrix for Z
gui = SimpleGUI("rotation")
rotationZ_slider = gui.add_slider("rotationZ", min_value=-90, max_value=90, initial_value=0)


# Create the rotation matrix for Y
rotationY_slider = gui.add_slider("rotationY", min_value=-180, max_value=180, initial_value=0)


# Create the rotation matrix for X
rotationX_slider = gui.add_slider("rotationX", min_value=-90, max_value=90, initial_value=0)
...
while draw:
    ...
    zRotationMatrix = m4.create_from_z_rotation(np.deg2rad(rotationZ_slider.get_value()))
    yRotationMatrix = m4.create_from_y_rotation(np.deg2rad(rotationY_slider.get_value()))
    xRotationMatrix = m4.create_from_x_rotation(np.deg2rad(rotationX_slider.get_value()))

    # Combine the translation, scaling, and rotation matrices to create a single model matrix.
    model_mat = m4.multiply(translation_matrix, xRotationMatrix)
    model_mat = m4.multiply(model_mat, yRotationMatrix)
    model_mat = m4.multiply(model_mat, zRotationMatrix)
    model_mat = m4.multiply(model_mat, scaling_matrix)
